...
useEffect(() => {
  ...
  const handlePostPrompt = () => setSellOpen(true);
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-4 md:gap-5 flex-shrink-0">
          ...
          <button
            ...
            title="Saved"
          >
            <img src="/icons/cop.png" alt="" style={{ width: "100%", height: "100%" }} />
          </button>
  ...
  <DropdownMenuTrigger asChild>
    ...
    >
      <Bell className="w-5 h-5 text-white" />
       {realUnreadCount> 0 && (
        <span
          className="absolute -top-1 -right-1 bg-red-500 text-white text-xs w-5 h-5 grid place-items-center rounded-full"
        >
...
>
  ...
  {cart.length > 0 && (
    <span className="absolute -top-1 -right-1 bg-pink-500 text-white text-xs w-5 h-5 grid place-items-center rounded-full">
      {cart.length}
    </span>
  )}
</button>
          ...
          <button
            ...
            className="inline-flex items-center justify-center gap-2 px-3 shrink-0 whitespace-nowrap"
            style={{ height: 36, borderRadius: 200, background: "#D9D9D9" }}
          >
            <span
              aria-hidden
            ...
            <span style={uiTextStyle} className="text-black">
              Upload Prompt
            </span>
          </button>
  ...
  >
    <div className="flex flex-col gap-3">
      ...
      <button
  type="button"
  // open on mousedown + eat the event so it can't click the CTA behind
  onMouseDown={(e) => {
    e.preventDefault();   // prevents focus shift / click-through
    e.stopPropagation();  // don't bubble to parents
    setProfileOpen(true); // open the modal
  }}
  // extra guard so no click leaks if it fires
  onClick={(e) => {
    e.preventDefault();
    e.stopPropagation();
  }}
