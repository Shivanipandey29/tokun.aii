...
import { ShoppingCart, Upload, Star, Trash } from "lucide-react";
...
export default function PromptHistory() {
  ...
  const initialTab = (searchParams.get("p") === "purchased" ? "purchased" : "uploaded") as
    | "purchased"
    | "uploaded";
  const [tab, setTab] = useState<"purchased" | "uploaded">(initialTab);

  // Keep tab in sync if URL param changes externally (e.g., via Header navigation)
  useEffect(() => {
    const p = searchParams.get("p");
    if (p === "purchased" || p === "uploaded") {
      setTab(p);
    }
  }, [searchParams]);

  // When user clicks tabs inside page, update URL (?p=)
  const handleTabChange = (v: string) => {
    const next = (v === "purchased" ? "purchased" : "uploaded") as "purchased" | "uploaded";
    setTab(next);
...
const mapped: Prompt[] = (body.purchases || []).map((p: any) => {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#07080A] text-white">
      ...
      <main className="flex-1">
        <div className="mx-auto w-full max-w-[1280px] px-4 md:px-6 pt-12 pb-20">
          {/* Header */}
          ...
          <Tabs value={tab} onValueChange={handleTabChange} className="w-full">
              <SectionHeader
    title={tab === "purchased" ? "Purchased History" : "Upload History"}
    subtitle={
      tab === "purchased"
  ...
  />
            <div className="flex items-center justify-center">
              <TabsList className="grid grid-cols-2 w-[340px] bg-white/5 border border-white/10 rounded-full p-1">
                <TabsTrigger
                  value="purchased"
                  className={[
            ...
            <TabsContent value="purchased" className="mt-0">
             <PlanBanner
  planLabel="Active Membership"
  planName={user?.plan ? `${user.plan.charAt(0).toUpperCase() + user.plan.slice(1)} Plan` : "Free Plan"}
