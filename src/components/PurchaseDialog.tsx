...
export default function PurchaseDialog({ open, onOpenChange, prompt, onPurchaseComplete }: PurchaseDialogProps) {
  ...
  const handlePurchase = async () => {
    ...
    try {
      const res = await fetch("http://localhost:5000/api/prompt/purchase", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ user_id: user.id, prompt, plan: selectedPlan, billingCycle, customerType, payMethod, charge: Number(finalPrice) }),
      });
