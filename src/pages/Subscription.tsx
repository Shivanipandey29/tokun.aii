...
const startEnterprisePurchase = async () => {
  ...
  return (
    <div className="min-h-screen bg-[#07080A] text-white">
      ...
      <main className="container mx-auto max-w-[1100px] px-4 py-10 md:py-14">
        {/* Title */}
        <div className="text-center max-w-[780px] mx-auto space-y-2">
          <h1 className='font-["Inter"] font-semibold text-[24px] leading-[1]'>
            Flexible Subscription Plans for Every Need
          </h1>
          <p className='font-["Inter"] text-[12.5px] leading-[1]'>
            Select the plan that perfectly fits your tokun optimization goals.
          </p>
        </div>

        {/* Billing toggle */}
        <div className="mt-6 mb-10 flex items-center justify-center gap-4 sm:gap-6 text-[12px]">
        ...
        {/* Cards */}
        <div className="mt-0 grid grid-cols-1 sm:grid-cols-[repeat(3,250px)] justify-center gap-x-[8px] gap-y-[8px]">
          <PlanCard
 ...
 if (!creating) setSelected("Enterprise");
   startEnterprisePurchase();
        ...
        <ComparisonTable />
        <p className='text-center text-[11px] mt-6 font-["Inter"] leading-[1]'>{note ?? " "}</p>
      </main>
  ...
  return (
    <Card
      ...
      style={{
        ...
        background: selected ? SELECTED_CARD_BG : "#0D0D0E", // use only 'background' to avoid React warning
        color: "#FFFFFF",
      }}
    >
      {highlight && (
          ...
          <ul className="mt-4 space-y-2 mx-auto">
            {extras.map((e) => {
              ...
              return (
                ...
                >
                  {negative ? <X className="h-[14px] w-[14px]" /> : <Check className="h-[14px] w-[14px]" />}
                  <span className='font-["Inter"] text-[12px] leading-[1]'>{e.label}</span>
                  <span className='font-["Inter"] text-[12px] leading-[1]'>:</span>
                  <span className='font-["Inter"] text-[12px] leading-[1] font-medium'>{e.value}</span>
                </li>
              );
            })}
          </ul>
...
function ComparisonTable() {
  ...
  const cell = (v: Cell) => {
    if (typeof v === "boolean") {
      return v ? <Check className="inline-block h-4 w-4" /> : <X className="inline-block h-4 w-4" />;
    }
    return v;
  };
  ...
  const row = (label: string, free: Cell, pro: Cell, ent: Cell) => (
    <tr className="border-t border-white/10">
      <td className='py-3 pr-4 text-left text-[12px] font-["Inter"]'>{label}</td>
    ...
    <div className="mt-8 overflow-x-auto">
      <table className="w-full text-center text-sm text-white">
        ...
        <tbody>
          ...
          {row(
            "History",
            <span className='text-[11px] font-["Inter"]'>(up to 5 entries)</span>,
            <span className='text-[11px] font-["Inter"]'>(unlimited entries)</span>,
            <span className='text-[11px] font-["Inter"]'>(unlimited entries)</span>
          )}
          {row(
            "Token Usage (Counted monthly only)",
            <span className='text-[11px] font-["Inter"]'>(section-wise)</span>,
            <span className='text-[11px] font-["Inter"]'>(section-wise)</span>,
            <span className='text-[11px] font-["Inter"]'>(section-wise)</span>
          )}
          {row(
          ...
          {row(
            "Phone Support",
            <span className='text-[11px] font-["Inter"]'>9:00–18:00, MON–SUN</span>,
            <span className='text-[11px] font-["Inter"]'>9:00–18:00, MON–SUN</span>,
            <span className='text-[11px] font-["Inter"]'>24/7 Dedicated Support</span>
          )}
        </tbody>
      </table>
    </div>
